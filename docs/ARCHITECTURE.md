...
- **rules/analyzers** apply the heuristics shipped in `cspbuilder/core.py`.
- **score** ranks by severity.
- **MCP server** (`cspbuilder mcp`) exposes `scan` for Cognis.Studio agents.
